...
final class AppSettings {
    ...
    var hapticsEnabled: Bool {
        get {
            UserDefaults.standard.object(forKey: "pantomim.hapticsEnabled")
                == nil
                ? true
                : UserDefaults.standard.bool(forKey: "pantomim.hapticsEnabled")
        }
        set {
            UserDefaults.standard.set(
                newValue,
                forKey: "pantomim.hapticsEnabled"
            )
        }
    }
    ...
    var hasSeenOnboarding: Bool {
        ...
        set {
            UserDefaults.standard.set(
                newValue,
                forKey: "pantomim.hasSeenOnboarding"
            )
        }
    }
    ...
    private func makeBuzzPlayer() -> AVAudioPlayer? {
        ...
        appendLE(UInt32(UInt32(sampleRate) * 2))
        appendLE(UInt16(2))
        appendLE(UInt16(16))
    ...
    func startPartyMusic() {
        ...
        if let url = Bundle.main.url(forResource: "party", withExtension: "mp3")
        {
            ...
            musicTask = Task { @MainActor in
                let sounds: [SystemSoundID] = [
                    1103, 1104, 1103, 1104, 1103, 1103, 1104, 1103,
                ]
                var i = 0
            ...
            ForEach(AppLanguage.allCases, id: \.self) { lang in
                Button {
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text(lang.label).font(
                            AppFonts.rounded(13, weight: .bold)
                        )
                    }
                    .padding(.horizontal, 11).padding(.vertical, 7)
                    .foregroundStyle(
                        language == lang ? Color.white : AppColors.textSecondary
                    )
                    .background {
                        if language == lang { Capsule().fill(AppColors.text) }
                    }
                }
            }
        }
...
struct StepperRow: View {
    ...
    var body: some View {
        HStack {
            Text(label).font(AppFonts.rounded(15, weight: .medium))
                ...
                Button {
                    ...
                    .foregroundStyle(
                        value > range.lowerBound
                            ? color : AppColors.textSecondary.opacity(0.3)
                    )
                }
                Text("\(value)").font(AppFonts.rounded(18, weight: .bold))
                    .foregroundStyle(AppColors.text)
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(title).font(AppFonts.rounded(15, weight: .bold))
                    .foregroundStyle(AppColors.text)
                Text(subtitle).font(AppFonts.rounded(12)).foregroundStyle(
...
struct PointsBadge: View {
    ...
    var body: some View {
        HStack(spacing: 3) {
            Image(systemName: "star.fill").font(
