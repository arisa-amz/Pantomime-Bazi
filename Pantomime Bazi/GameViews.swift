    ...
    var twoTeamBar: some View {
        HStack(spacing: 0) {
            ForEach(Array(vm.settings.teams.enumerated()), id: \.element.id) {
                ...
                VStack(spacing: 3) {
                    Text(team.icon).font(.system(size: 24))
                    Text(team.name)
                        .font(AppFonts.rounded(18, weight: .heavy))
                        .foregroundStyle(isActive ? vm.currentTeam.color : vm.currentTeam.color).lineLimit(1)
                    Text("\(team.totalScore)")
                        .font(
                            .system(size: 22, weight: .black, design: .rounded)
                        )
                        .foregroundStyle(isActive ? vm.currentTeam.color : vm.currentTeam.color)
                }
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ...
                { i, team in
                    let isActive = i == vm.currentTeamIndex
                    HStack(spacing: 6) {
                        Text(team.icon).font(.system(size: 44))
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                Button {
                        ...
                        .font(
                            .system(
                                size: selectedPoints == 0
                                    && vm.turnCategories.isEmpty ? 16 : 16,
                                weight: .black,
                                design: .rounded
                            )
                        ).tracking(0.5)
                    ...
                    .foregroundStyle(
                        canStart
                            ? actingTeam.color : actingTeam.color.opacity(0.45)
                    )
                    ...
                    Button {
                        ...
                        ZStack {
                            VStack(spacing: 3) {
                                ...
                                .font(AppFonts.rounded(12, weight: .bold))
                                .foregroundStyle(isOn ? vm.currentTeam.color : .white)
                        ...
                        ZStack {
                            ...
                            .shadow(
                                color: (!isBlocked && isSelected)
                                    ? AppColors.forPoints(pts).opacity(0.8)
                                    : .clear,
                                radius: 6,
                                y: 3
                            )
